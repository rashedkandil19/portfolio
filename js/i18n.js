(() => {
  ...
  function applyLanguage(lang) {
    const current = SUPPORTED_LANGS.includes(lang) ? lang : DEFAULT_LANG;
  ...
  window.viaWhatsApp = function () {
    ...
    window.open(`https://wa.me/201044310426?text=${encodeURIComponent(message)}`, "_blank");
  };

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", loadTranslations);
  } else {
    loadTranslations().catch(err => console.error("i18n error:", err));
  }
})();
